/*!
*\file
*\brief Данный  файл содержит в себе реализацию основных функций класса Records, которые используются в программе.
*/
...
void Records::checkRecords (const QString& strRecords)
{
    ...
    for(int i = 0; i < strOneRecord.count(); i++)
    {
        ...
        {
            throw QString ("Текст записей:\nОшибка! У свойства записи целочисленные значения записываются между '[' и ']' (количество '[' и ']' должно совпадать)."
                           "\nОшибка в записи " + QString::number(i + 1) + ": " + strOneRecord[i]);
        }
        ...
        {
            throw QString ("Текст записей:\nОшибка! Запись должна иметь хотя бы одно свойство."
