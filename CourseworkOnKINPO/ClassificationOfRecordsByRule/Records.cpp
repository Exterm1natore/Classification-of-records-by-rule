...
QString Records::checkRecords (const QString& strRecords)
{
    ...
    while (checkStart != -1 && checkEnd != -1 && checkEnd > checkStart && strRecords.count("[") == strRecords.count("]")) // пока находимся внутри скобок
    {
            ...
            if (currentString == otherString)
            {
                return "Ошибка! Названия записей не могут повторяться."
                       "\nНазвание записи: \"" + currentString + "\""
                        "\nПовторение в записях: " + QString::number(i + 1) + " и " + QString::number(j + 1); // Найдено повторяющиеся строки
            }
        }
    }
