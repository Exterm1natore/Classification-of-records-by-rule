...
QString FileHandling::unpackTextFile(const QString& filePath)
{
    ...
    if(!regex.match(content).hasMatch())
    {
        ...
        throw QString("Ошибка! Входной текст из файла может содержать только буквы русского или английского алфавита, "
                      "а также следующий набор символов: квадратные скобки ('[' ']'), точка с запятой (';'), двоеточие (':'), "
                      "равно ('='), точку ('.'), двойные кавычки (' \" '), пробел, запятая (','), цифры [1;99] и не может быть пустым. "
                      "\nФайл: " + filePath);
    }
